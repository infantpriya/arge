...
// It receives the "General Inquiry" form data from the website, emails YOU the inquiry,
...
const FROM_EMAIL = "Angel Roch Global Exports <inquiries@angelrochexports.com>"; // must be on a domain verified in Resend
...
export default async function handler(req, res) {
  ...
  const notifyToYou = {
    ...
    to: [TO_EMAIL],
    reply_to: email,
    subject: `New website inquiry from ${name}${company ? " (" + company + ")" : ""}`,
    html: `
  ...
  const autoReplyToBuyer = {
    from: FROM_EMAIL,
    to: [email],
    subject: "We received your inquiry — Angel Roch Global Exports",
    html: `
